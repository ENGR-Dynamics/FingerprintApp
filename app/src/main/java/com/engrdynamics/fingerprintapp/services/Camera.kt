 ...
 */
package com.engrdynamics.fingerprintapp.services
...
import com.engrdynamics.fingerprintapp.utils.camera.getCaptureSize
import com.engrdynamics.fingerprintapp.utils.camera.getPreviewSize
import com.engrdynamics.fingerprintapp.utils.camera.isAutoExposureSupported
import com.engrdynamics.fingerprintapp.utils.camera.isContinuousAutoFocusSupported
...
interface ImageHandler {
    fun handleImage(image: Image, id : Int): Runnable
}
...
class Camera constructor(private val cameraManager: CameraManager) {
    ...
    private val captureCallback = object : CameraCaptureSession.CaptureCallback() {
        private fun process(result: CaptureResult) {
            ...
            lastFocus = result.get(CaptureResult.LENS_FOCUS_DISTANCE)!!.toDouble()
        }
    ...
    fun takePicture(handler: ImageHandler) {
        ...
        imageReader?.setOnImageAvailableListener({ reader ->
            ...
            backgroundHandler?.post(handler.handleImage(image = image, id = 1))
        }, backgroundHandler)

        imageReader2?.setOnImageAvailableListener({ reader ->
            val image = reader.acquireNextImage()
            backgroundHandler?.post(handler.handleImage(image = image, id = 2))
        }, backgroundHandler)
    ...
    private fun startDualCamera(surfaces: List<Surface>) {
        val size = cameraManager.getCameraCharacteristics(physicalCameraIds.toList()[0]).getCaptureSize(CompareSizesByArea())
        val size2 = cameraManager.getCameraCharacteristics(physicalCameraIds.toList()[1]).getCaptureSize(CompareSizesByArea())
        imageReader = ImageReader.newInstance(size.width, size.height, ImageFormat.JPEG, 2)
        imageReader2 = ImageReader.newInstance(size2.width, size2.height, ImageFormat.JPEG, 2)
        val outputConfigs = (surfaces + imageReader?.surface!! + imageReader2?.surface!!).mapIndexed { index, surface ->
            val physicalCameraId = physicalCameraIds.toList()[index % 2]
    ...
    private fun enableDefaultModes(builder: CaptureRequest.Builder?) {
        builder?.apply {
            ...
            Log.e("DEBUG", characteristics.get(LENS_INFO_MINIMUM_FOCUS_DISTANCE).toString());
            //set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF);
            //set(CaptureRequest.LENS_FOCUS_DISTANCE, characteristics.get(LENS_INFO_MINIMUM_FOCUS_DISTANCE));
            ...
            val rect = characteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE)
            val newRect = Rect(rect!!.right / 2 - 100, rect!!.bottom / 2 - 100, rect!!.right / 2 + 100, rect!!.bottom / 2 + 100)
            // Adjust focus area with metering weight
            val meteringRectangle = MeteringRectangle(newRect, 500)
            builder.set(CaptureRequest.CONTROL_AF_REGIONS, arrayOf(meteringRectangle))
            builder.set(CaptureRequest.CONTROL_AE_REGIONS, arrayOf(meteringRectangle))
    ...
    private fun captureStillPicture() {
        ...
        try {
            ...
            builder?.addTarget(imageReader2!!.surface)
            //builder?.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH);
            //builder?.set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_AUTO);
            //builder?.set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO);
            builder?.set(CaptureRequest.SENSOR_EXPOSURE_TIME, 100);
            //surfaces?.forEach {
